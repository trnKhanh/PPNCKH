...
class OtsuThresholdLayer(tf.keras.layers.Layer):
    ...
    def otsu_converter(self, inputs):
        ...
        for gray_image in gray_images:
            threshold = threshold_otsu(tf.squeeze(gray_image).numpy())
    ...
    def call(self, inputs):
        return tf.reshape(tf.py_function(self.otsu_converter, [inputs], tf.int32),tf.shape(inputs))
