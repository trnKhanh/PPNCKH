...
if __name__ == "__main__":
    ...
    name = "mobilev2_ImageCLEF12_dual_sobel_oversampling"
    ...
    for X, _ in train_dataset.take(1):
        ...
        plt.axis(False)
        plt.savefig("leaf_img.png")

        gray = tf.image.grayscale_to_rgb(tf.image.rgb_to_grayscale(X))
        plt.figure(figsize=(10,10))
        plt.imshow(gray[0].numpy().astype("uint8"))
        plt.axis(False)
        plt.savefig("gray_img.png")

        sobel = sobel_edge(X)
        plt.figure(figsize=(10,10))
        plt.imshow(sobel[0].numpy().astype("uint8"))
        plt.axis(False)
        plt.savefig("sobel_img.png")
